...
const PersonalProjects = ({
  ...
  return (
    ...
    >
      ...
      />
      {active === name && (
        <div className="absolute p-8 justify-start w-full bottom-0 flex-col bg-[rgba(0,0,0,0.8)] rounded-b-[24px] max-sm:rounded-[24px] max-sm:h-full font-robot text-white">
          <div className="flex justify-between items-center">
            <h3 className="text-2xl font-bold">{name}</h3>
            <div className="flex gap-4">
              {/* External link icon - Needed to change styles, so no image is used */}
              <motion.a href={href} target="_blank" whileHover={{ scale: 1.2 }}>
                <svg
